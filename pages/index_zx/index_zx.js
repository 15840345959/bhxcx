...
Page({ 
  ...
  getNewbihua: function () {
    ...
    util.getNewbihua(param, function (res) {
      console.log("Newbihuaaaaa : " + JSON.stringify(res))
      if (res.data.code == "200" && res.data.result == true) {
        var Newbihua2 = [];
        var Newbihua = res.data.ret.data;
        var Newbihua1 = Newbihua;
        Newbihua2 = vm.data.Newbihua;
        for (var i in Newbihua1) {//循环出最新壁画所有信息
          Newbihua2.push(Newbihua1[i])
        }
        vm.init()
        vm.setData({
          Newbihua: Newbihua2
        ...
        page++;
        console.log("Newbihua:" + JSON.stringify(vm.data.Newbihua));
      } else {
